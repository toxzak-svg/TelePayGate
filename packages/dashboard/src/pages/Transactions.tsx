...
export default function Transactions() {
  ...
  const items: Payment[] = paymentsResp?.data ?? [];
  const total = paymentsResp?.meta?.total ?? items.length;
    ...
    <div>
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex gap-3">
          <button
            onClick={() => exportToCsv('payments.csv', items as unknown as Record<string, unknown>[])}
        ...
        <div className="bg-white rounded-lg shadow p-6 overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {items.map((tx: Payment) => (
                <tr key={tx.id}>
                  <td className="px-4 py-2 font-mono text-sm text-blue-700">{tx.id}</td>
                  <td className="px-4 py-2">{tx.userId || tx.telegramUserId}</td>
                  <td className="px-4 py-2">${tx.amount}</td>
                  <td className="px-4 py-2 font-semibold">{tx.status}</td>
                  <td className="px-4 py-2 text-xs text-gray-500">{new Date(tx.createdAt).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
