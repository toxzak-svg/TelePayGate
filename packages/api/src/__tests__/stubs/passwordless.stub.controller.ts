...
export const PasswordlessStubController = {
  ...
  async verifyMagicLink(req: Request, res: Response, _next?: NextFunction) {
    ...
    if (!email) return res.status(400).json({ success: false, error: { code: 'INVALID_TOKEN' } });
    ...
    res.cookie('session_id', sessionToken, { httpOnly: true, maxAge: 24 * 60 * 60 * 1000 });
  ...
  async me(req: Request, res: Response, _next?: NextFunction) {
    ...
    if (!session) return res.status(401).json({ success: false, error: { code: 'INVALID_SESSION' } });
    if (session.expiresAt < Date.now()) return res.status(401).json({ success: false, error: { code: 'EXPIRED' } });
