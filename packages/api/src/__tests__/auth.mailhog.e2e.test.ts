...
process.env.FEATURE_PASSWORDLESS_AUTH =
...
describe("Magic Link via MailHog (E2E)", () => {
  ...
  it("issues magic link and verifies via MailHog", async () => {
    ...
    const mailhogUrl =
      process.env.MAILHOG_API_URL || "http://localhost:8025/api/v2/messages";
    ...
    while (!token && Date.now() < deadline) {
      ...
      for (const m of msgs) {
        const recipients = (m.To || []).map(
          (r: { Mailbox: string; Domain: string }) =>
            r.Mailbox + "@" + r.Domain,
        );
        if (recipients.includes(testEmail)) {
          ...
          const rawBody =
            content.Body ||
            (content.Headers &&
              (content.Headers["HTML"] || content.Headers["Text"])) ||
            "";
          ...
          console.log(
            "MailHog message body did not contain token for",
            testEmail,
          );
        }
      }
