...
describe("Backup Codes Integration", () => {
  ...
  beforeAll(async () => {
    ...
    if (!dbUrl)
      throw new Error("DATABASE_URL must be set for backup codes test");
    initDatabase(dbUrl);
    ...
    const user = await db.one(
      "INSERT INTO dashboard_users (email, role, is_active, created_at, updated_at) VALUES ($1, $2, true, now(), now()) RETURNING id",
      ["test-backup@example.com", "developer"],
    );
    userId = user.id;
  ...
  test("should persist and verify backup codes", async () => {
    ...
    const db = getDatabase();
    const rows = await db.any("SELECT * FROM backup_codes WHERE user_id = $1", [
      userId,
    ]);
