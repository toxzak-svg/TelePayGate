...
describe("Passwordless Auth (Isolated or Fixture)", () => {
  ...
  beforeAll(async () => {
    if (process.env.USE_TESTCONTAINERS === "true") {
      ...
      try {
        ...
        console.error(
          "[test-debug] startPostgresFixture error:",
  ...
  it("should request and verify magic link, then access /auth/me", async () => {
    ...
    if (process.env.USE_TESTCONTAINERS === "true") {
      console.log(
        "[test-debug] magic-link response:",
        res1.status,
    ...
    if (process.env.USE_TESTCONTAINERS === "true") {
      console.log(
        "[test-debug] verify response:",
        res2.status,
        JSON.stringify(res2.body),
        "headers:",
        res2.headers && res2.headers["set-cookie"],
      );
    }
