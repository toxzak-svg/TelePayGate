...
export function requireDashboardRole(role: 'admin' | 'editor' | 'viewer' | 'dev') {
  return async function (req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!dashboardUserId) {
        ...
        const sessionId = cookieHeader?.split(';').map(c=>c.trim()).find(c=>c.startsWith('session_id='))?.split('=')[1];
      ...
      const rolesHierarchy: Record<string, number> = { viewer: 1, dev: 2, editor: 3, admin: 4 };
