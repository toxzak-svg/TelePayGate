...
export function responseMiddleware(
  ...
  res.replySuccess = (data?: unknown, status = 200) => {
    ...
    console.warn(
      "[DEPRECATED] `res.replySuccess` is deprecated. Use `respondSuccess` (utils/response) instead. See docs/process/response-helpers.md",
    );
  ...
  res.replyError = (
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "[DEPRECATED] `res.replyError` is deprecated. Use `respondError` (utils/response) instead. See docs/process/response-helpers.md",
    );
