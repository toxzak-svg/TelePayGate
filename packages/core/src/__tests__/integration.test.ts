...
describe("Conversion and Reconciliation Integration Test", () => {
  ...
  beforeEach(() => {
    ...
    tonService =
      new (TonBlockchainService as jest.Mock<TonBlockchainService>)() as jest.Mocked<TonBlockchainService>;
  ...
  it("should process a conversion and then reconcile it", async () => {
    ...
    (conversionService as any).p2pLiquidityService = {
      ...
      executeConversion: jest
        .fn()
        .mockResolvedValue({ success: true, txHash: "tx-hash" }),
    };
    ...
    // --- Mock DB ---
    const _dbOneSpy = jest
      .spyOn(db, "one")
      .mockResolvedValue({ total_stars: 1000 });
    const _dbNoneSpy = jest.spyOn(db, "none").mockResolvedValue(undefined);
    const dbOneOrNoneSpy = jest
      .spyOn(db, "oneOrNone")
      .mockResolvedValue({
        id: conversionId,
        target_amount: 1,
    ...
    expect(dbOneOrNoneSpy).toHaveBeenCalledWith(
      expect.stringContaining("SELECT * FROM conversions"),
      [conversionId],
    );
  });
});
