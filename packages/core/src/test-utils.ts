...
export function setupDexIntegrationTest() {
  ...
  if (!runDexIntegrationTests) {
    console.warn('⚠️ Skipping DEX integration tests (set RUN_DEX_INTEGRATION_TESTS=true to enable).');
  } else if (process.env.DEX_SIMULATION_MODE === 'true') {
    console.log('🧪 DEX integration tests running in simulation mode');
  }

  process.env.DEDUST_API_URL = process.env.DEDUST_API_URL || 'https://api.dedust.io';
  process.env.STONFI_API_URL = process.env.STONFI_API_URL || 'https://api.ston.fi';
